...
```{r}
#Remove factor variables
data <- select(labeleddata, subset = -c("label", "bikeorcar"))
```
...
N start should be big enough but don't need to try multiple values
...
cluster2
```
The sample point from cluster two is high in PC1 and mid range in PC2. It has high weight, speed, and overall total. It is a car. 
